...
app.controller("myController", function ($scope, $http) { 
    ...
    $scope.flashSalesTime = function () {			
        $http.get('/api/flashSales').then(resp => {
    ...
    $scope.saveData = function () {
      ...
      var firstTimeFormatted = (firstTime.getHours() > 12 ? firstTime.getHours() - 12 : firstTime.getHours()) + ':' + (firstTime.getMinutes() < 10 ? '0' : '') + firstTime.getMinutes() + (firstTime.getHours() >= 12 ? ' PM' : ' AM');
      var lastTimeFormatted = (lastTime.getHours() > 12 ? lastTime.getHours() - 12 : lastTime.getHours()) + ':' + (lastTime.getMinutes() < 10 ? '0' : '') + lastTime.getMinutes() + (lastTime.getHours() >= 12 ? ' PM' : ' AM');
      ...
      $http.post('/api/saveFlashSales', dataToSend).then(response => {
					//  alert("ngon");/**/
          $scope.flashSalesTime();// lấy dữ 
				}).catch(error => {
					console.error(error);
